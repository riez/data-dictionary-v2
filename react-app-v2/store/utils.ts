...
export function findRoleNamesInMetadata(
  ...
  if (!metadata || !metadata.sources?.[0].tables) return ["admin"]

  const allRoles = metadata.sources?.[0].tables
    .flatMap(table =>
      [
...
export function getColumnAliasIfExists(params: GetColumnAliasParams) {
  const metadataTable = params.metadata?.sources?.[0].tables?.find(
...
export function groupMetadataAndPostgresInfoByTableName(
  ...
  if (!params.metadata || !params.metadata.sources?.[0].tables) return null
  ...
  const combinedTables = params.metadata.sources?.[0].tables.map(metadataTable => {
